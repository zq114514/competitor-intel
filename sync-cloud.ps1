...
function Get-Bytes($it) {
  if ($null -eq $it.Local -or -not (Test-Path -LiteralPath $it.Local)) { return ,([byte[]]@()) }
  try { $raw = [IO.File]::ReadAllBytes($it.Local) } catch { return ,([byte[]]@()) }
  if ($null -eq $raw -or $raw.Length -eq 0) { return ,([byte[]]@()) }
  # 二进制检测：含 NUL (0x00) 字节或扩展名属于图片/字体/压缩包 → 不做 CRLF 归一化
  ...
  $isBinary = ($raw | Where-Object { $_ -eq 0 } | Select-Object -First 1) -or ($binaryExts -contains $ext)
